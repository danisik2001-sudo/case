...
class RaffleDayController extends Controller
{
    ...
    public function getRaffle(Request $request)
    {
        ...
        $minDep = $settings->raffle_dep_day;

        // Получаем текущий розыгрыш с типом 'day'
        $RaffleActual = Raffle::getActualDayRaffle(); // Исправили вызов метода

        $getRedisUsers = empty($this->redis->lrange('day:' . $RaffleActual->id . ':users', 0, -1))
            ? false
            : $this->redis->lrange(
                'day:' . $RaffleActual->id . ':users',
                0,
                -1
            );
        ...
        if (!Auth::guest()) {
            $place = 0;
            foreach ($this->redis->lrange('day:' . $RaffleActual->id . ':users', 0, -1) as $i) {
                $place++;
        ...
        // Получаем последний розыгрыш с типом 'day'
        $RaffleLast = Raffle::where('status', 1)->where('type', 'day')->orderBy('id', 'desc')->first();

        if ($RaffleLast) {
            $winners = Raffle::where('status', 1)->where('type', 'day')->orderBy('id', 'desc')->take(15)->get()
                ->map(function ($raffle) {
    ...
    public function RaffleOpen()
    {
        ...
        $game = Raffle::getActualDayRaffle();
        $place = 1;
        foreach (
            $this->redis->lrange('day:' . $game->id . ':users', 0, -1)
            as $i
        ) {
            $place++;
    ...
    {
        $RaffleActual = Raffle::getActualDayRaffle();
        if (date("Y-m-d H:i:s", strtotime('-' . 24 . 'hours')) < $RaffleActual->created_at) {
            return response()->json([
                'success' => false,
                'message' => 'Raffle day cannot be completed yet',
            ], 400);
        }
        ...
        if (count($user) > 0) {
            ...
            $this->redis->del('day:' . $RaffleActual->id . ':users');
